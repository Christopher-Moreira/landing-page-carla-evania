...
import { Instagram, Facebook, MessageCircleMore, Linkedin } from "lucide-react";
...
export default function SocialLandingPage() {
  ...
  const redesSociais = [
    ...
    { icon: <MessageCircleMore className="w-12 h-12 text-green-400 mb-4" />, title: "Whatsapp", desc: "Agendamentos Gerais são melhores pelo meu whatsapp.", link: "https://wa.me/5555996991914" },
  ...
  return (
    <div className="font-sans bg-pink-200 text-gray-900 flex flex-col min-h-screen overflow-x-hidden">
      ...
      <section id="redes-sociais" className="py-8 px-4 text-center w-full">
          ...
          {redesSociais.map((rede, i) => (
            ...
            >
              <CardHeader className="flex-1 flex flex-col items-center">
                {rede.icon}
                <CardTitle className="text-xl text-pink-800">{rede.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-gray-700 text-sm">{rede.desc}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <Footer className="bg-pink-200 mt-21" container style={{ backgroundColor: "#fbcfe8" }}>
        <FooterCopyright 
